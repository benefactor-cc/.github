...
Synchronize shared history with ordinary merge commits: avoid git rebase in favor of git merge. Never rewrite or discard either local or remote commits to manufacture parity.
...
3. Review linked pull requests, issues, Linear work, related repositories in `benefactor-cc`, and relevant external-organization repositories whenever behavior or contracts cross boundaries.
